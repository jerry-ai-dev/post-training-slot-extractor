...
### 1.1 核心功能
本项目要训练一个模型，来负责 Smart Appointment AI Agent 项目中的「按摩预约」模块。它读取用户与机器人多轮交互产生的预约对话历史、工具结果以及当前最新一轮用户输入，提取性别、到店时间、时长、按摩偏好、指定技师和用户确认反馈等信息，并输出固定 Schema 的 JSON。业务只有按摩这一固定服务类别，因此模型合同中不设置 `project`；肩颈、精油、足部、泰式等需求统一归入 `preferences`。此项目虽然和智能预约系统关联，但独立成为一个项目，你可以完全不关心这个 Agent 项目，只记住我们该训练的核心目标：输入是多轮用户对话和工具状态，输出是指定 JSON 格式或工具调用。
...
当上下文信息已经足够完整，例如预约时间、时长、所选技师及其可用性都已经明确时，模型应该直接输出最终 JSON。这里考察的是：模型能否从多轮对话和工具结果中提取出正确字段，并保证 JSON 格式稳定。
...
当时间和时长已经明确，但还不知道指定技师是否存在/可约，或还需要按偏好搜索可用技师时，模型不应该直接猜最终 JSON，而应该输出工具调用。这里考察的是：模型能否判断当前轮次需要调用工具，以及参数是否正确。
...
| `technician_name` | 姓名字符串或 `null` |
| `missing_info` | 仅允许 `start_time,duration_minutes`，并按此顺序输出 |

未知单值一律使用 JSON `null`，禁止输出字符串“未知”“无”“不限”。除 `preferences` 外，所有字段和工具参数均按类型和值严格匹配；模型输出“女”而不是 `female`、输出“1小时”而不是整数 `60`，都属于未遵循合同。
...
* **消除幻觉**：强力抑止小模型容易凭空编造、脑补列表中不存在的技师名字与服务项目的幻觉行为（对用户没有提到、候选列表中也不存在的信息严格输出 `null`）。
* **上下文意图理解**：在复杂、多轮的应答确认路径中，在极短、多义的字词回复下精准分类用户意图。
* **工具调用准确性**：在需要核实技师存在性、档期或搜索候选时，能够判断是否需要调用工具，并从上下文中提取正确参数。
...
**类型 1：信息不全 → 追问澄清**
必填项还缺（如缺时间或时长）。判定逻辑：对照工具调用前提发现有缺口。关键决策：输出最终 JSON，但 `technician_status=not_checked`、`info_complete=false`，并在 `missing_info` 列出还缺哪些项（本项目不单设 `action=ask`）。
...
> `info_complete` 只表示用户必填预约条件是否收齐。`start_time` 和 `duration_minutes` 均已知时为 `true`；工具返回无可用结果不会把它改回 `false`。技师状态由 `technician_status` 独立表达。
...
从效果目标看，模型需要读取 `history`、`user_input`、`current_time`、技师工具结果和工具描述等上下文，输出固定 JSON 或工具调用 JSON。它要解决的是高频、低开放度、强格式约束的任务，而不是长篇开放问答。因此，模型的通用知识规模不是第一优先级，**指令遵循、中文理解、结构化输出和边界控制**更重要。
...
**一、中文口语预约理解能力。** 用户输入不是标准表单，而是"明天下午想按个肩颈""安排个女师傅""小王有空吗""那换一个吧"这类自然口语。模型需要理解按摩预约场景中的时间、时长、按摩偏好、性别偏好、技师名和确认/否定表达。尤其是中文相对时间表达非常关键，例如"明天""后天下午""周末""三点半"必须结合 `current_time` 标准化，而不是凭模型记忆乱猜。
...
**四、工具调用决策能力。** 本项目虽然不是完整 Function Calling Agent，但训练样本里有明确的 `action=tool_call`、`tool_name` 和 `arguments`。模型要学会什么时候追问、什么时候调用 `find_technicians`、什么时候基于工具结果输出最终 JSON。这里需要的是轻量决策能力，不是长链路复杂规划能力。

**五、幻觉抑制能力。** 模型不能编造技师、档期和服务项目。凡是数据库、排班才知道的事实，都必须交给工具。基座模型如果本身指令遵循较弱，后续即使微调，也更容易出现"为了补全 JSON 而脑补字段"的问题。
...
除了速度，还需要单独考虑模型上下文窗口是否能容纳一次完整任务输入。这里的“上下文”不能只理解成用户当前一句话，而是一次推理请求中模型实际看到的全部内容：系统提示词、任务规则、JSON Schema、工具描述、多轮对话历史、当前用户输入、当前时间、技师候选、工具返回结果，以及模型即将生成的输出。
...
- **成本（次要但加分）**：偏好对可由"标准答案 vs 程序化构造的错误样本"自动批量生成（编造工具结果外技师或档期、短词误判闲聊、该调工具却直接输出最终 JSON、工具参数抽错），几乎零额外标注成本。这是锦上添花，不是唯一理由。
...
> rejected 的来源是**程序化构造**：把 chosen 按"典型错误模式"做定向扰动——编造工具结果外技师或档期（打 P4）、把多义短词判成闲聊或判错 confirmation（打 P5）、该 tool_call 却直接输出最终 JSON（打 P6/P7）、工具参数抽错或漏填（打 P6）、JSON 被 Markdown 包裹/字段缺失（打 P1，可选）。这样几乎零额外标注成本（呼应 2.4.3）。
...
| 工具调用（查技师） | 300 | ~21% | **过采样**（自然偏低，抬到与高频类齐平） |
...
|---|---|---|
| P4 幻觉（编造列表外信息） | ~40%（主力） | 把 chosen 改成编造工具结果外的技师或档期，该填 `null` 却乱填 |
...
| **幻觉率**（越低越好） | 技师是否有当前输入证据支持：调用参数/未核实时来自用户，已可约时来自最近工具结果 | 规则（证据一致性检测） | P4 |
...
>
> 评估和真实推理中，`find_technicians` 每一轮都作为可用能力提供；是否调用必须由模型依据上下文判断。不得只在 expected 为 `tool_call` 的样本里暴露工具，否则会把标准动作泄漏进输入。
>
...
```jsonc
{
  ...
  "expected": { "action": "final", "preferences": ["精油"], "start_time": "2026-06-10 14:00", "duration_minutes": 60 },
  "assertions": [
...
- **执行检查（关键工具兜底）**：对 `find_technicians` 补少量"真的调用一次"，看返回是否合理，防止静态看着对、实际调起来报错。
